...
func _import_json(json: Dictionary) -> void:
	global_profiles = json[K_PROFILE_LOCATIONS].map(func(profile_path: String) -> Profile : return Profile.new(profile_path))

func _export_json(json: Dictionary) -> void:
	json.merge({
		K_PROFILE_LOCATIONS: global_profiles.map(func(profile: Profile) -> String : return profile.save_path)
	})
...
static func _internal_add_profile(profile: Profile) -> void:
	...
	prints("Profiles:", global_profiles)
